...
class AutthNotifier with ChangeNotifier {
  ...
  void login(LoginModel data, BuildContext ctx) async {
    ...
    try {
      ...
      var url =
          Uri.parse('https://fc676bed094a.ngrok-free.app/auth/token/login');
  ...
  void register(RegisterModel data, BuildContext ctx) async {
    ...
    try {
      log(data.toJson().toString());
      var url = Uri.parse('https://fc676bed094a.ngrok-free.app/auth/users/');
      log(url.toString());
      var response = await http.post(
        url,
        headers: {
  ...
  void getUser(String accessToken, BuildContext ctx) async {
    setLoading(true);
    try {
      var url = Uri.parse('https://fc676bed094a.ngrok-free.app/auth/users/me');
